...
{   
    ...
    internal static SpellCheckResult CheckString( string str, CancellationToken? token )
    {
        try
        {
            ...
            for ( int i = 0; i < words.Length; ++i )
            {
                if ( token?.IsCancellationRequested ?? false)
                    return new() { State = SpellCheckResultState.Cancelled };
            ...
            return new SpellCheckResult() { State = SpellCheckResultState.Success, Words = results.ToArray() };
        }
        catch ( Exception ) { return new() { State = SpellCheckResultState.Exception }; } // Fail silently.
    }
}
